...
do
    ...
    step Running sql2csv.py -C config.py distribucio_de_socies.sql --data "${data}"
    rm -f "$result"
    sql2csv.py -C config.py distribucio_de_socies.sql --data "${data}" > "$result" 2>&1 && diff "$expect" "$result" && ok "$result" || ko "$result"
done
