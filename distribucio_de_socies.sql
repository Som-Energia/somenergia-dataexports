...
FROM(
	...
	WHERE adreces.partner_id IS NOT NULL
		...
		AND adreces.create_date < '2015-09-01') as taula_provincia
...
(SELECT comunitat_autonoma AS nombre, count(comunitat_autonoma) AS recuento
FROM(
	SELECT adreces.id, 
		adreces.partner_id, 
		municipi.name AS municipi, 
...
(SELECT comarca AS nombre, count(comarca) AS recuento
FROM(
	SELECT adreces.id, 
		adreces.partner_id, 
		municipi.name AS municipi, 
...
(SELECT pais AS nombre, count(pais) AS recuento
FROM(
	SELECT adreces.id, 
		adreces.partner_id, 
		municipi.name AS municipi, 
